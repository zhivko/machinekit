...
STATIC inline int tpAddSegmentToQueue(TP_STRUCT * const tp,
		TC_STRUCT * const tc, int inc_id);

STATIC inline double tpGetMaxTargetVel(TP_STRUCT const * const tp,
		TC_STRUCT const * const tc);

STATIC int tpAdjustAccelForTangent(TP_STRUCT const * const,
		TC_STRUCT * const tc, double normal_acc_scale);
/**
 ...
 */
STATIC int tpRotaryMotionCheck(TP_STRUCT const * const tp,
		TC_STRUCT const * const tc) {
	...
	case TC_CIRCULAR:
		if (tc->coords.circle.abc.tmag_zero
				&& tc->coords.circle.uvw.tmag_zero) {
			return false;
		} else {
			return true;
		}
...
STATIC int tpGetMachineAccelBounds(TP_STRUCT const * const tp,
		PmCartesian * const acc_bound) {
	if (!acc_bound) {
		return TP_ERR_FAIL;
	}
...
STATIC int tpGetMachineVelBounds(TP_STRUCT const * const tp,
		PmCartesian * const vel_bound) {
	if (!vel_bound) {
		return TP_ERR_FAIL;
	}
...
STATIC int tpGetMachineActiveLimit(double * const act_limit,
...
STATIC inline double tpGetRealTargetVel(TP_STRUCT const * const tp,
	...
	return rtapi_fmin(v_target * tpGetFeedScale(tp, tc),
			tpGetMaxTargetVel(tp, tc));
}
 ...
 */
STATIC inline double tpGetSignedSpindlePosition(double spindle_pos,
		int spindle_dir) {
	if (spindle_dir < 0.0) {
		spindle_pos *= -1.0;
	}
...
int tpSetCurrentPos(TP_STRUCT * const tp, EmcPose const * const pos) {
	...
	if (emcPoseValid(pos)) {
		...
		rtapi_print_msg(RTAPI_MSG_ERR,
				"Tried to set invalid pose in tpSetCurrentPos on id %d!"
						"pos is %.12g, %.12g, %.12g\n", tp->execId, pos->tran.x,
...
int tpAddCurrentPos(TP_STRUCT * const tp, EmcPose const * const disp) {
	...
	if (emcPoseValid(disp)) {
		...
		rtapi_print_msg(RTAPI_MSG_ERR,
				"Tried to set invalid pose in tpAddCurrentPos on id %d!"
						"disp is %.12g, %.12g, %.12g\n", tp->execId,
 ...
 */
STATIC double tpCalculateTriangleVel(TP_STRUCT const * const tp,
		TC_STRUCT * const tc) {
 ...
 * halfway point.
 */
STATIC double tpCalculateOptimizationInitialVel(TP_STRUCT const * const tp,
		TC_STRUCT * const tc) {
	double acc_scaled = tpGetScaledAccel(tp, tc);
...
STATIC int tpInitBlendArcFromPrev(TP_STRUCT const * const tp,
		TC_STRUCT const * const prev_tc, TC_STRUCT* const blend_tc, double vel,
	...
	tcInit(blend_tc, TC_SPHERICAL, canon_motion_type, tp->cycleTime,
			prev_tc->enables, prev_tc->atspeed);
	...
	tcSetupMotion(blend_tc, vel, ini_maxvel, acc);
...
STATIC int tcSetLineXYZ(TC_STRUCT * const tc, PmCartLine const * const line) {
	...
	if (!tc->coords.line.abc.tmag_zero || !tc->coords.line.uvw.tmag_zero) {
		rtapi_print_msg(RTAPI_MSG_ERR,
				"SetLineXYZ does not supportABC or UVW motion\n");
		return TP_ERR_FAIL;
	}
...
STATIC int tpCreateLineArcBlend(TP_STRUCT * const tp, TC_STRUCT * const prev_tc,
	...
	int res_init = blendInit3FromLineArc(&geom, &param, prev_tc, tc, &acc_bound,
			&vel_bound, get_maxFeedScale(tp->shared));
	...
	int res_post = blendLineArcPostProcess(&points_exact, &points_approx,
			&param, &geom, &prev_tc->coords.line.xyz, &tc->coords.circle.xyz);
	...
	blendCheckConsume(&param, &points_exact, prev_tc,
			get_arcBlendGapCycles(tp->shared));
	...
	int res_stretch1 = pmCartLineStretch(&line1_temp, new_len1, false);

	double phi2_new = tc->coords.circle.xyz.angle - points_exact.trim2;

	tp_debug_print("phi2_new = %f\n",phi2_new);
	int res_stretch2 = pmCircleStretch(&circ2_temp, phi2_new, true);
	//TODO create blends
	...
	pmCartLinePoint(&line1_temp, line1_temp.tmag, &points_exact.arc_start);
	pmCirclePoint(&circ2_temp, 0.0, &points_exact.arc_end);
	//TODO deal with large spiral values, or else detect and fall back?

	blendPoints3Print(&points_exact);
	int res_arc = arcFromBlendPoints3(&blend_tc->coords.arc.xyz, &points_exact,
			&geom, &param);
	if (res_arc < 0) {
	...
	tpInitBlendArcFromPrev(tp, prev_tc, blend_tc, param.v_req, param.v_plan,
			param.a_max);
	blend_tc->target_vel = param.v_actual;

	int res_tangent = checkTangentAngle(&circ2_temp, &blend_tc->coords.arc.xyz,
			&geom, &param, tp->cycleTime, true);
	...
	if (tpCheckTangentPerformance(tp, prev_tc, tc, blend_tc)
			== TP_ERR_NO_ACTION) {
		return TP_ERR_NO_ACTION;
	}
...
STATIC int tpCreateArcLineBlend(TP_STRUCT * const tp, TC_STRUCT * const prev_tc,
		TC_STRUCT * const tc, TC_STRUCT * const blend_tc) {
	...
	int res_init = blendInit3FromArcLine(&geom, &param, prev_tc, tc, &acc_bound,
			&vel_bound, get_maxFeedScale(tp->shared));
	...
	int res_post = blendArcLinePostProcess(&points_exact, &points_approx,
			&param, &geom, &prev_tc->coords.circle.xyz, &tc->coords.line.xyz);
	...
	int res_stretch1 = pmCircleStretch(&circ1_temp, phi1_new, false);
	if (res_stretch1 != TP_ERR_OK) {
		return TP_ERR_FAIL;
	}

	double new_len2 = tc->target - points_exact.trim2;
	int res_stretch2 = pmCartLineStretch(&line2_temp, new_len2, true);
	...
	pmCirclePoint(&circ1_temp, circ1_temp.angle, &points_exact.arc_start);

	pmCartLinePoint(&line2_temp, 0.0, &points_exact.arc_end);
	...
	int res_tangent = checkTangentAngle(&circ1_temp, &blend_tc->coords.arc.xyz,
			&geom, &param, tp->cycleTime, false);
	if (res_tangent) {
		tp_debug_print("failed tangent check, aborting arc...\n");
		return TP_ERR_FAIL;
	}
...
STATIC int tpCreateArcArcBlend(TP_STRUCT * const tp, TC_STRUCT * const prev_tc,
		TC_STRUCT * const tc, TC_STRUCT * const blend_tc) {
	...
	int res_init = blendInit3FromArcArc(&geom, &param, prev_tc, tc, &acc_bound,
			&vel_bound, get_maxFeedScale(tp->shared));
	...
	int res_post = blendArcArcPostProcess(&points_exact, &points_approx, &param,
			&geom, &prev_tc->coords.circle.xyz, &tc->coords.circle.xyz);
	...
	int res_tangent1 = checkTangentAngle(&circ1_temp, &blend_tc->coords.arc.xyz,
			&geom, &param, tp->cycleTime, false);
	int res_tangent2 = checkTangentAngle(&circ2_temp, &blend_tc->coords.arc.xyz,
			&geom, &param, tp->cycleTime, true);
...
STATIC int tpCreateLineLineBlend(TP_STRUCT * const tp,
		TC_STRUCT * const prev_tc, TC_STRUCT * const tc,
		TC_STRUCT * const blend_tc) {
	...
	int res_init = blendInit3FromLineLine(&geom, &param, prev_tc, tc,
			&acc_bound, &vel_bound, get_maxFeedScale(tp->shared));
	...
	blendCheckConsume(&param, &points, prev_tc,
			get_arcBlendGapCycles(tp->shared));

	// Set up actual blend arc here
	int res_arc = arcFromBlendPoints3(&blend_tc->coords.arc.xyz, &points, &geom,
			&param);
	if (res_arc < 0) {
		return TP_ERR_FAIL;
	}
...
STATIC int tpCheckCanonType(TC_STRUCT * const prev_tc,
		TC_STRUCT const * const tc) {
	if (!tc || !prev_tc) {
		return TP_ERR_FAIL;
	}
 ...
 */
int tpAddRigidTap(TP_STRUCT * const tp, EmcPose end, double vel,
	...
	pmRigidTapInit(&tc.coords.rigidtap, &tp->goalPos, &end);
...
STATIC int tpComputeOptimalVelocity(TP_STRUCT const * const tp,
		TC_STRUCT * const tc, TC_STRUCT * const prev1_tc) {
...
STATIC int tpRunOptimization(TP_STRUCT * const tp) {
	...
	for (x = 1; x < get_arcBlendOptDepth(tp->shared) + 2; ++x) {
		...
		if (!tc->finalized) {
			...
			prev1_tc->finalvel = rtapi_fmin(prev1_tc->maxvel,
					tpCalculateOptimizationInitialVel(tp, tc));
			tc->finalvel = 0.0;
		} else {
...
STATIC double pmCartAbsMax(PmCartesian const * const v) {
	return rtapi_fmax(rtapi_fmax(rtapi_fabs(v->x), rtapi_fabs(v->y)),
			rtapi_fabs(v->z));
}
...
STATIC int tpAdjustAccelForTangent(TP_STRUCT const * const tp,
	...
	if (normal_acc_scale >= 1.0) {
		rtapi_print_msg(RTAPI_MSG_ERR,
				"Can't have acceleration scale %f > 1.0\n", normal_acc_scale);
		return TP_ERR_FAIL;
	}
 ...
 * rate.
 */
STATIC int tpSetupTangent(TP_STRUCT const * const tp, TC_STRUCT * const prev_tc,
		TC_STRUCT * const tc) {
	if (!tc || !prev_tc) {
	...
	const double SHARP_CORNER_THRESHOLD = rtapi_cos(
			PM_PI * (1.0 - SHARP_CORNER_DEG / 180.0));
	...
	double v_max1 = rtapi_fmin(prev_tc->maxvel,
			prev_tc->reqvel * get_maxFeedScale(tp->shared));
	double v_max2 = rtapi_fmin(tc->maxvel,
			tc->reqvel * get_maxFeedScale(tp->shared));
	...
	tp_debug_print("acc_diff: %f %f %f\n",
			acc_diff.x,
			acc_diff.y,
			acc_diff.z); tp_debug_print("acc_scale: %f %f %f\n",
			acc_scale.x,
			acc_scale.y,
			acc_scale.z);
...
int tpAddCircle(TP_STRUCT * const tp, EmcPose end, PmCartesian center,
		...
		double ini_maxvel, double acc, unsigned char enables, char atspeed,
		struct state_tag_t tag) {
	if (tpErrorCheck(tp) < 0) {
		return TP_ERR_FAIL;
	}
	...
	tcInit(&tc, TC_CIRCULAR, canon_motion_type, tp->cycleTime, enables,
			atspeed);
	tc.tag = tag;
	// Setup any synced IO for this move
	tpSetupSyncedIO(tp, &tc);
	...
	int res_init = pmCircle9Init(&tc.coords.circle, &tp->goalPos, &end, &center,
			&normal, turn);
	...
	double v_max_actual = pmCircleActualMaxVel(&tc.coords.circle.xyz,
			ini_maxvel, acc, false);

	// Copy in motion parameters
	tcSetupMotion(&tc, vel, v_max_actual, acc);
...
STATIC int tpComputeBlendVelocity(TP_STRUCT const * const tp,
	...
	double v_reachable_this = rtapi_fmin(tpCalculateTriangleVel(tp, tc),
			target_vel_this);
	double v_reachable_next = rtapi_fmin(tpCalculateTriangleVel(tp, nexttc),
			target_vel_next);
	...
	if (tc->tolerance > 0) {
		...
		if (rtapi_cos(theta) > min_cos_theta) {
			tblend_vel = 2.0
					* pmSqrt(acc_this * tc->tolerance / rtapi_cos(theta));
 ...
 */
STATIC int tcUpdateDistFromAccel(TC_STRUCT * const tc, double acc,
		double vel_desired) {
	...
	tc->on_final_decel = (rtapi_fabs(vel_desired - tc->currentvel)
			< TP_VEL_EPSILON) && (acc < 0.0);

	return TP_ERR_OK;
}

STATIC void tpDebugCycleInfo(TP_STRUCT const * const tp,
		TC_STRUCT const * const tc, TC_STRUCT const * const nexttc, double acc) {
#ifdef TC_DEBUG
...
void tpCalculateTrapezoidalAccel(TP_STRUCT const * const tp,
	...
	tc_debug_print("Klemen using trapezoidal acceleration\n");
	...
	double discr_term2 = maxaccel
			* (2.0 * dx - tc->currentvel * tc->cycle_time);
...
void tpToggleDIOs(TP_STRUCT const * const tp, TC_STRUCT * const tc) {

	unsigned int i = 0;
	if (tc->syncdio.anychanged != 0) { // we have DIO's to turn on or off
		for (i = 0; i < get_num_dio(tp->shared); i++) {
			if (!(RTAPI_BIT_TEST(tc->syncdio.dio_mask, i)))
				continue;
			if (tc->syncdio.dios[i] > 0)
				dioWrite(tp->shared, i, 1); // turn DIO[i] on
			if (tc->syncdio.dios[i] < 0)
				dioWrite(tp->shared, i, 0); // turn DIO[i] off
		}
		for (i = 0; i < get_num_aio(tp->shared); i++) {
			if (!(tc->syncdio.aio_mask & (1ull << i)))
				continue;
 ...
 */
STATIC void tpUpdateRigidTapState(TP_STRUCT * const tp, TC_STRUCT * const tc) {

	double new_spindlepos = get_spindleRevs(tp->shared);
	if (get_spindle_direction(tp->shared) < 0)
	...
	case TAPPING:
		...
		if (tc->progress >= tc->coords.rigidtap.reversal_target) {
			// command reversal
			set_spindle_speed(tp->shared, get_spindle_speed(tp->shared) * -1.0);
 ...
 */
STATIC int tpUpdateMovementStatus(TP_STRUCT * const tp,
		TC_STRUCT const * const tc) {
...
STATIC void tpUpdateBlend(TP_STRUCT * const tp, TC_STRUCT * const tc,
	...
	if (tpGetFeedScale(tp, nexttc) > TP_VEL_EPSILON) {
		...
		double blend_progress = rtapi_fmax(rtapi_fmin(dv / vel_start, 1.0),
				0.0);
...
STATIC void tpHandleEmptyQueue(TP_STRUCT * const tp) {
	...
	set_enables_queued(tp->shared, get_enables_new(tp->shared));
}
 ...
 */
STATIC int tpCompleteSegment(TP_STRUCT * const tp, TC_STRUCT const * const tc) {
...
STATIC int tpActivateSegment(TP_STRUCT * const tp, TC_STRUCT * const tc) {
	 ...
	 * performance cost.
	 * */
	double cutoff_time = 1.0
			/ (rtapi_fmax(get_arcBlendRampFreq(tp->shared), TP_TIME_EPSILON));
	...
	double segment_time = 2.0 * length
			/ (tc->currentvel
					+ rtapi_fmin(tc->finalvel, tpGetRealTargetVel(tp, tc)));

	tp_debug_print("segment_time = %f, cutoff_time = %f, motionType=%d, termCond=%d\n",segment_time, cutoff_time,tc->canon_motion_type,tc->term_cond);
	...
	if (TC_SYNC_POSITION == tc->synchronized
			&& !(get_spindleSync(tp->shared))) {
 ...
 */
STATIC void tpSyncVelocityMode(TP_STRUCT * const tp, TC_STRUCT * const tc,
		TC_STRUCT const * nexttc) {
...
STATIC void tpSyncPositionMode(TP_STRUCT * const tp, TC_STRUCT * const tc,
	...
	double pos_desired = (tp->spindle.revs - tp->spindle.offset)
			* tc->uu_per_rev;
...
STATIC int tpUpdateCycle(TP_STRUCT * const tp, TC_STRUCT * const tc,
	...
	tp_debug_print("Klemen: If the slowdown is not too great, use velocity ramping instead of trapezoidal velocity\nAccelMode:%d,tangent?%b",tc->accel_mode,(tc->term_cond == TC_TERM_COND_TANGENT));
...
STATIC inline int tcSetSplitCycle(TC_STRUCT * const tc, double split_time,
	...
	if (tc->splitting != 0 && split_time > 0.0) {
		rtapi_print_msg(RTAPI_MSG_ERR,
				"already splitting on id %d with cycle time %.16g, dx = %.16g, split time %.12g\n",
				tc->id, tc->cycle_time, tc->target - tc->progress, split_time);
		return TP_ERR_FAIL;
	}
	tc->splitting = 1;
...
int tpSetAout(TP_STRUCT * const tp, unsigned int index, double start,
		double end) {
	if (0 == tp) {
		return TP_ERR_FAIL;
	}
...
int tpSetDout(TP_STRUCT * const tp, unsigned int index, unsigned char start,
		unsigned char end) {
	if (0 == tp) {
		return TP_ERR_FAIL;
	}
